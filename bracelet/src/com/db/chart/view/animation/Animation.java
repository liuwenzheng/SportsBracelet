...
public class Animation {
	...
	/**
	 * Animation global duration Likely to be removed in future.
	 */
	private long mGlobalDuration;

	/**
	 * Keeps the current global duration Likely to be removed in future.
	 */
	private long mCurrentGlobalDuration;

	/**
	 * Keeps the global initial time of the animation Likely to be removed in
	 * future.
	 */
	...
	/**
	 * Keeps the information regarding whether points will be animated in
	 * parallel or in sequence.
	 */
	private float mOverlapingFactor;

	/**
	 * Factor from 0 to 1 to specify where does the animation starts according
	 * innerchart area.
	 */
	 ...
	 * 
	 * @param chartView
	 *            - {@link ChartView} to invalidate each time the animation
	 *            wants to update the values.
	 * @param startY
	 *            - starting point of values.
	 * @param sets
	 *            - Array of {@link ChartSet} containing the target values.
	...
	public ArrayList<ChartSet> prepareEnter(ChartView chartView,
			ArrayList<float[]> startingX, ArrayList<float[]> startingY,
		...
		// Calculates the expected duration as there was with no overlap (factor
		// = 0)
		float noOverlapDuration = mGlobalDuration / mSets.get(0).size();
		// Adjust the duration to the overlap
		mDuration = (int) (noOverlapDuration + (mGlobalDuration - noOverlapDuration)
				* mOverlapingFactor);
		...
		for (int i = 0; i < mSets.size(); i++) {
			...
			for (int j = 0; j < mSets.get(i).size(); j++) {
				...
				path.lineTo(mSets.get(i).getEntry(j).getX(), mSets.get(i)
						.getEntry(j).getY());
		...
		for (int i = 0; i < mInitTime.length; i++) {
			// Calculates the expected init time as there was with no overlap
			// (factor = 0)
			noOverlapInitTime = mGlobalInitTime
					+ (i * (mGlobalDuration / mSets.get(0).size()));
	...
	public ArrayList<ChartSet> prepareEnter(ChartView chartView,
		...
		if (mStartXFactor != -1)
			x = chartView.getInnerChartLeft()
					+ (chartView.getInnerChartRight() - chartView
							.getInnerChartLeft()) * mStartXFactor;

		float y = 0;
		if (mStartYFactor != -1)
			y = chartView.getInnerChartBottom()
					- (chartView.getInnerChartBottom() - chartView
							.getInnerChartTop()) * mStartYFactor;
		else
			y = chartView.getInnerChartBottom();

		final ArrayList<float[]> startXValues = new ArrayList<float[]>(
				sets.size());
		final ArrayList<float[]> startYValues = new ArrayList<float[]>(
				sets.size());
		float[] Xset;
		float[] Yset;
		...
		return prepareEnter(chartView, startXValues, startYValues, sets);
	}
		...
		for (int i = 0; i < mSets.size(); i++)
			...
			for (int j = 0; j < mSets.get(i).size(); j++) {
				...
				mSets.get(i).getEntry(j)
						.setCoordinates(posUpdate[0], posUpdate[1]);
			}
	 ...
	 * 
	 * @param currentTime
	 *            - value from 0 to 1 representing the current time of the
	 *            animation
	...
	private float[] getEntryUpdate(int i, int j, float normalizedTime) {
		...
		if (mPathMeasures[i][j].getPosTan(mPathMeasures[i][j].getLength()
	 ...
	 * 
	 * @param factor
	 *            - value from 0 to 1 that tells how much will be the overlap of
	 *            an entry's animation according to the previous one. 0 - no
	 *            overlap 1 - all entries animate in paralell (default)
	 */
	 ...
	 *            overlap 1 - all entries animate in paralell (default)
	 * @param order
	 *            - order from which the entries will be animated. { 0, 1, 2, 3,
	 *            ...} - default order
	 */
	 ...
	 * 
	 * @param runnable
	 *            to be executed once the animation finishes.
	 */
	 ...
	 * 
	 * @param xFactor
	 *            - horizontal factor between 0 and 1
	 * @param yFactor
	 *            - vertical factor between 0 and 1 Eg. xFactor=0; yFactor=0;
	 *            starts the animation on the bottom left corner of the inner
	 *            chart area.
	 */
	 ...
	 * 
	 * @param speed
	 *            - speed of alpha animation values according with translation.
	 *            To disable alpha set it to -1. Eg. If speed 2 alpha goes twice
