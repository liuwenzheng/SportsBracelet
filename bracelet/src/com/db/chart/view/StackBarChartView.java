...
public class StackBarChartView extends BarChartView {
	...
	public void onDrawChart(Canvas canvas, ArrayList<ChartSet> data) {
		...
		for (int i = 0; i < data.get(0).size(); i++) {
			...
			if (style.hasBarBackground)
				drawBarBackground(canvas, data.get(0).getEntry(i).getX()
						- barWidth / 2);
			...
			// Unfortunately necessary to discover which set is the bottom and
			// top
			...
			for (int j = 0; j < data.size(); j++) {
				...
				if (j == bottomSetIndex) {
					canvas.drawRoundRect(
							...
							style.cornerRadius, style.barPaint);

					if (bottomSetIndex != topSetIndex
						...
						canvas.drawRect(
								new Rect(
										...
										(int) (this.getInnerChartBottom()
												- (dist + verticalOffset) + cornersFix)),
								style.barPaint);
					}
					...
					canvas.drawRect(new Rect((int) (bar.getX() - barWidth / 2),
							...
							(int) (bar.getX() + barWidth / 2),
							(int) (nextBottomY)), style.barPaint);

				} else {// if(j != bottomSetIndex && j != topSetIndex){ //
						// Middle sets
					canvas.drawRect(
							new Rect(
									(int) (bar.getX() - barWidth / 2),
	...
	private static int discoverBottomSet(int entryIndex,
	...
	public void onPreDrawChart(ArrayList<ChartSet> data) {
		...
		if (data.get(0).size() == 1)
			...
			calculateBarsWidth(data.get(0).getEntry(0).getX(), data.get(0)
					.getEntry(1).getX());
	}
